...
export function startPluginRemote(mainFilename: string, pluginId: string, peerSend: (message: RpcMessage, reject?: (e: Error) => void, serializationContext?: any) => void, startPluginRemoteOptions?: StartPluginRemoteOptions) {
    ...
    attachPluginRemote(peer, {
        ...
        async onLoadZip(scrypted: ScryptedStatic, params: any, packageJson: any, getZip: () => Promise<Buffer>, zipOptions: PluginRemoteLoadZipOptions) {
            ...
            const onProxySerialization = (value: any, proxyId: string, sourceAddress?: string, sourcePort?: number) => {
            ...
            const resolveObject = async (id: string, sourcePeerAddress: string, sourcePeerPort: number) => {
                const sourcePeer = sourcePeerPort
                    ? await clusterPeers.get(getClusterPeerKey(sourcePeerAddress, sourcePeerPort))
                    : peer;
            ...
            const clusterRpcServer = net.createServer(client => {
                ...
                const clusterPeerKey = getClusterPeerKey(client.remoteAddress, clusterPeerPort);
                clusterPeer.onProxySerialization = (value, proxyId) => onProxySerialization(value, proxyId,clusterPeerAddress, clusterPeerPort);
                ...
                const connectRPCObject: ConnectRPCObject = async (o) => {
                    ...
                    return resolveObject(o.proxyId, o.address, o.sourcePort);
                }
            ...
            const ensureClusterPeer = (address: string, connectPort: number) => {
                ...
                if (!clusterPeerPromise) {
                    clusterPeerPromise = (async () => {
                        const socket = net.connect(connectPort, address || '127.0.0.1');
                        ...
                        try {
                            ...
                            clusterPeer.tags.localAddress = sourceAddress;
                            clusterPeer.tags.localPort = sourcePort;
                            clusterPeer.onProxySerialization = (value, proxyId) => onProxySerialization(value, proxyId, sourceAddress, sourcePort);
                            return clusterPeer;
                        }
                        catch (e) {
            ...
            scrypted.connectRPCObject = async (value: any) => {
                ...
                if (clusterObject?.id !== clusterId)
                    return value;
                const { address, port, proxyId, sourceAddress, sourcePort } = clusterObject;
                ...
                if (address === SCRYPTED_CLUSTER_ADDRESS && port === clusterPort)
                    return resolveObject(proxyId, address, sourcePort);
                ...
                try {
                    ...
                    if (clusterPeer.tags.localAddress === sourceAddress && clusterPeer.tags.localPort === sourcePort)
                        return value;
